...
Let's also define $h$ as the \textbf{hypothesis}. That is, $h$ is a function that maps from $x's$ to a estimated value. In the example of Table~\ref{tab:training-set} \textbf{$h$ is a hypothesis that maps from the size of a house to an estimated price}
